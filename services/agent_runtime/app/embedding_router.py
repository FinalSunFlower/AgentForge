...
TOOL_PROTOTYPES: tuple[tuple[str, str], ...] = (
    (
        "intent_router",
        "Classify the request intent or adapter: math, science, data, audio, or general text.",
    ),
    (
        "readonly_sql",
        "Run a read-only SQL SELECT analytics query against the novels database.",
    ),
    (
        "passive_sonar",
        "Triangulate a passive sonar source from sensor bearings in degrees.",
    ),
    (
        "wind_tunnel",
        "Compute inviscid potential-flow pressure around a cylinder in a wind tunnel.",
    ),
    (
        "calculator",
    ...
    (
        "retrieval",
        "Search, retrieve, find, or look up published notes, chapters, primers, and authors.",
    ),
    (
        "handoff",
        "Transfer this run to the science specialist or the retrieval specialist with a brief.",
    ),
)
...
SPECIALIST_PROTOTYPES: tuple[tuple[str, str], ...] = (
    (
        "science",
        "Science specialist for calculate and compute arithmetic expressions, sonar triangulation, and wind-tunnel cylinder flow.",
    ),
    (
        "retrieval",
...
def select_tool_names(prompt: str) -> list[str]:
    ...
    if _contains_any(prompt, _RETRIEVAL_HINTS):
        kept = [name for name in kept if name not in {"passive_sonar", "wind_tunnel"}]
...
INTENT_PROTOTYPES: tuple[tuple[str, str], ...] = (
    ...
    ("science", "Sonar triangulation, bearings, or a wind-tunnel cylinder experiment."),
    ("data", "Read-only SQL analytics query against a database."),
    ("audio", "Audio, music, singer, or voice request."),
)
